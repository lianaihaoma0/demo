...
@implementation NSTextAttachment (JLText)

+ (instancetype)checkBoxAttachment{
    NSTextAttachment *textAttachment = [[NSTextAttachment alloc] init];
    textAttachment.bounds = CGRectMake(0, 0, 20, 20);
    textAttachment.image = [self imageWithType:JLParagraphTypeCheckbox];
...
+ (UIImage *)imageWithType:(JLParagraphType)type {
...
- (JLTextAttachmentType)attachmentType {
    return [(NSNumber *)objc_getAssociatedObject(self, keyOfAttachmentType) intValue];
}

- (void)setAttachmentType:(JLTextAttachmentType)attachmentType {
